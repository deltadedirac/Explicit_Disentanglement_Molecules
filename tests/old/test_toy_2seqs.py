...
def preprocessing_toy(dataset_msa, ref_msa, c2i, i2c, i2i):
    ...
    '''    
    ref_msa = ref_msa.float(); ref_msa[:,1]=torch.tensor([0.25]*3)
    ref_msa = ref_msa.float(); ref_msa[:,3]=torch.tensor([0.25]*3)    
    ref_msa = ref_msa.float(); ref_msa[:,5]=torch.tensor([0.25]*3)
    '''

    x1 = dataset_msa.prot_space

    #pad = torch.ones(2,1,3)*0.33
    ...
    del c2i['-']
    #dataset_msa.prot_space = torch.cat((x1,pad),1)
    #dataset_msa.padded_idx = [[3],[3]]
    #dataset_msa.non_padded_idx =  [[0,1,2],[0,1,2]]
...
if __name__ == '__main__':
    ...
    path = std.parserinfo('*/PathOrig2seq')
    ...
    pdb.set_trace()
    # Optimizer
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-7) #lr=1e-3 and weigh_dacay1e-6 - 1e-7 in that range is working for such examples
