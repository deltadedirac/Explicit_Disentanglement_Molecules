...
class VITAE_CI(nn.Module):
    ...
    def forward(self, x, eq_samples=1, iw_samples=1, switch=1.0,trainprocess=False):
        ...
        '''-------------------------------------------------------------------------------------------------------'''
        # "Detransform" output
        self.stn.st_gp_cpab.interpolation_type = 'linear' #'GP' #'linear'
        ...
        # IMPORTANT!!!!!!!
        # THIS IS FOR TESTING IF CPAB IS WORKING FOR TRANSFORMING TO A GUIDED REFERENCE...
        # NECESARY TO USE A DECORATOR + PATTERN DESIGN TO MAKE MORE CLEAN THE CODE.
    ...
    def forward(self, x, eq_samples=1, iw_samples=1, switch=1.0,trainprocess=False):
        ...
        return x_mean.unsqueeze(1).contiguous(), \
                x_var.unsqueeze(1).contiguous(), [z1, z2], [mu1, mu2], [var1, var2], x_new, theta_mean
